...
@implementation TGBCacheManager
+ (TGBCacheManager *)sharedInstance {
    static dispatch_once_t once;
    static TGBCacheManager *_sharedInstance;
    dispatch_once(&once, ^{
        _sharedInstance = [[TGBCacheManager alloc] init];
    });
...
+ (NSString *)path {
    return [TGBPersistenceUtils avCacheDirectory];
}

- (NSString *)diskCachePath {
    if (!_diskCachePath) {
        _diskCachePath = [TGBCacheManager path];
    }
    ...
    dispatch_async(self.cacheQueue, ^{
        ...
        if (maxCacheAge<=0) {
            isTooOld=YES;
             ...
             diskResult=[TGBPersistenceUtils getJSONFromPath:[self pathForKey:[key AVMD5String]]];
            if (diskResult && maxCacheAge > 0) {
                NSDate *lastModified = [TGBPersistenceUtils lastModified:[self pathForKey:[key AVMD5String]]];
...
- (void)saveJSON:(id)JSON forKey:(NSString *)key {
    dispatch_async(self.cacheQueue, ^{
        [TGBPersistenceUtils saveJSON:JSON toPath:[self pathForKey:[key AVMD5String]]];
    });
}
...
+ (BOOL)clearAllCache {
    ...
    dispatch_sync([TGBCacheManager sharedInstance].cacheQueue, ^{
        NSString *path = [TGBCacheManager path];
...
+ (BOOL)clearCacheMoreThanOneDay {
    return [TGBCacheManager clearCacheMoreThanDays:1];
}
...
+ (BOOL)clearCacheMoreThanDays:(NSInteger)numberOfDays {
    ...
    dispatch_sync([TGBCacheManager sharedInstance].cacheQueue, ^{
        [TGBPersistenceUtils deleteFilesInDirectory:[TGBCacheManager path] moreThanDays:numberOfDays];
    });
