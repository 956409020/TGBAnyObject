...
@interface TGBScheduler ()
@property (nonatomic, strong) NSTimer *timer;
@property (nonatomic, assign) NSInteger firedCount;
@end

@implementation TGBScheduler

+ (TGBScheduler *)sharedInstance {
    static dispatch_once_t once;
    static TGBScheduler *_sharedInstance;
    dispatch_once(&once, ^{
        _sharedInstance = [[TGBScheduler alloc] init];
...
- (void)handleArchivedRequests {
    [[TGBPaasClient sharedInstance] handleAllArchivedRequests];
}

- (void)clearCache {
    [TGBCacheManager clearCacheMoreThanDays:self.queryCacheExpiredDays];
}
