...
@implementation TGBPersistenceUtils
...
+ (NSString *)cacheSandboxPath {
    NSString *applicationId = [TGBKKit getApplicationId];
...
// ~/Library/Caches/AVPaasCache, for TGBCacheManager
+ (NSString *)avCacheDirectory {
    NSString *ret = [[TGBPersistenceUtils homeDirectoryLibraryCaches] stringByAppendingPathComponent:@"AVPaasCache"];
...
+ (NSString *)privateDocumentsDirectory {
    NSString *ret = [[TGBPersistenceUtils libraryDirectory] stringByAppendingPathComponent:@"Private Documents/AVPaas"];
    [self createDirectoryIfNeeded:ret];
    return ret;
}
...
+ (NSString *)currentUserArchivePath {
    NSString * path = [[TGBPersistenceUtils privateDocumentsDirectory] stringByAppendingString:@"/currentUser"];
    return path;
}

+ (NSString *)currentUserClassArchivePath {
    NSString *path = [[TGBPersistenceUtils privateDocumentsDirectory] stringByAppendingString:@"/currentUserClass"];
    return path;
}

+ (NSString *)currentInstallationArchivePath {
    NSString *path = [[TGBPersistenceUtils privateDocumentsDirectory] stringByAppendingString:@"/currentInstallation"];
    return path;
}

+ (NSString *)eventuallyPath {
    NSString *ret = [[TGBPersistenceUtils privateDocumentsDirectory] stringByAppendingPathComponent:@"OfflineRequests"];
    [self createDirectoryIfNeeded:ret];
    return ret;
}
...
+ (BOOL)deleteFilesInDirectory:(NSString *)dirPath moreThanDays:(NSInteger)numberOfDays {
    ...
    if (error == nil) {
        for (NSString *path in directoryContents) {
            ...
            NSDate *lastModified = [TGBPersistenceUtils lastModified:fullPath];
