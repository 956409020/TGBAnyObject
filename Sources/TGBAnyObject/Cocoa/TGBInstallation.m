...
@implementation TGBInstallation

+ (TGBQuery *)query
{
    TGBQuery *query = [[TGBQuery alloc] initWithClassName:@"_Installation"];
    return query;
}

+(TGBQuery *)installationQuery
{
    TGBQuery *query = [[TGBQuery alloc] initWithClassName:[TGBInstallation className]];
    return query;
}
...
{
    TGBInstallation * installation = [[TGBInstallation alloc] init];
    return installation;
}

+ (TGBInstallation *)defaultInstallation
{
    static TGBInstallation *installation = nil;
    
    static dispatch_once_t onceToken;
    
    dispatch_once(&onceToken, ^{
        
        installation = [[TGBInstallation alloc] init];
    });
...
- (id)init
{
    self = [super init];
    if (self) {
        self.className  = [TGBInstallation className];
        self.deviceType = [TGBInstallation deviceType];
        self.timeZone   = [[NSTimeZone systemTimeZone] name];
        self.apnsTopic  = [NSBundle mainBundle].bundleIdentifier;
        
        NSString *path = [TGBPersistenceUtils currentInstallationArchivePath];
        if ([TGBPersistenceUtils fileExist:path]) {
            NSMutableDictionary *installationDict = [NSMutableDictionary dictionaryWithDictionary:[TGBPersistenceUtils getJSONFromPath:path]];
            if (installationDict) {
                [TGBObjectUtils copyDictionary:installationDict toObject:self];
            }
        }
    }
    return self;
...
- (void)saveInstallationToLocalCache {
    [TGBPersistenceUtils saveJSON:[self installationDictionaryForCache]
                          toPath:[TGBPersistenceUtils currentInstallationArchivePath]];
}

- (BOOL)isDirty {
    if ([super isDirty]) {
        return YES;
    } else if ([TGBInstallation defaultInstallation] == self) {
...
-(NSMutableDictionary *)updateInstallationDictionary:(NSMutableDictionary * )data
{
    [data addEntriesFromDictionary:@{
        badgeTag: @(self.badge),
        deviceTypeTag: [TGBInstallation deviceType],
    ...
    NSDictionary *updationData = [TGBObjectUtils dictionaryFromObject:localDataCopy];
...
{
    return [TGBInstallation defaultInstallation];
}
