...
-(void)queryWithBlock:(NSString *)path
    ...
    [[TGBPaasClient sharedInstance] getObject:path withParameters:parameters policy:self.cachePolicy maxCacheAge:self.maxCacheAge block:^(id object, NSError *error) {
        if (error == nil)
        {
            ...
            [TGBUtils callArrayResultBlock:resultBlock array:array error:nil];
        ...
        {
            [TGBUtils callArrayResultBlock:resultBlock array:nil error:error];
        }
    }];
}
...
- (NSMutableArray *)processResults:(NSArray *)results className:(NSString *)className {
    ...
    for(NSDictionary * dict in results)
    {
        TGBObject *object;
        if (className) {
            object = [TGBObjectUtils TGBObjectForClass:className];
        } else {
            object = [TGBObject objectWithClassName:@"SearchQuery"];
        }
