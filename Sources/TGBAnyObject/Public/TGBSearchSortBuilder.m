...
@implementation TGBSearchSortBuilder

+ (instancetype)newBuilder {
    TGBSearchSortBuilder *sortBuilder = [[[self class] alloc] init];
    return sortBuilder;
}
...
- (void)whereNear:(NSString *)key point:(TGBGeoPoint *)point {
    [self whereNear:key point:point inOrder:@"asc"];
}

- (void)whereNear:(NSString *)key point:(TGBGeoPoint *)point inOrder:(NSString *)order {
    [self whereNear:key point:point inOrder:order withMode:@"avg" andUnit:@"km"];
}

- (void)whereNear:(NSString *)key point:(TGBGeoPoint *)point inOrder:(NSString *)order withMode:(NSString *)mode andUnit:(NSString *)unit {
