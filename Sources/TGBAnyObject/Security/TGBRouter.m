...
{
    static TGBRouter *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[TGBRouter alloc] init];
    });
    return instance;
}
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        NSMutableDictionary *(^ loadCacheToMemoryBlock)(NSString *) = ^NSMutableDictionary *(NSString *key) {
            NSString *filePath = [[TGBRouter routerCacheDirectoryPath] stringByAppendingPathComponent:key];
            BOOL isDirectory;
...
{
    NSString *version = [TGBRouter APIVersion];
...
static NSString * TGBRouterCacheDirectoryPath = nil;

+ (void)setRouterCacheDirectoryPath:(NSString *)directoryPath
{
    TGBRouterCacheDirectoryPath = directoryPath;
}

+ (NSString *)routerCacheDirectoryPath
{
    if (!TGBRouterCacheDirectoryPath) {
        TGBRouterCacheDirectoryPath = [TGBPersistenceUtils homeDirectoryLibraryCachesLeanCloudCachesRouter];
    }
    return TGBRouterCacheDirectoryPath;
}
...
//            AVLoggerError(AVLoggerDomainDefault, @"%@", error);
    ...
    NSString *filePath = ({
        NSString *routerCacheDirectoryPath = [TGBRouter routerCacheDirectoryPath];
        BOOL isDirectory;
...
- (void)getAppRouterDataWithAppID:(NSString *)appID callback:(void (^)(NSDictionary *dataDictionary, NSError *error))callback
{
    ...
    [[TGBPaasClient sharedInstance] getObject:AppRouterURLString withParameters:@{@"appId":appID} block:^(id _Nullable object, NSError * _Nullable error) {
        if (error) {
            callback(nil, error);
        } else {
...
{
    return [self appURLForPath:path appID:[TGBKKit getApplicationId]];
}
...
{
    NSParameterAssert(appID);
    return [self appURLForPath:[TGBRouter RTMRouterPath] appID:appID];
}
...
- (void)getRTMRouterDataWithAppID:(NSString *)appID RTMRouterURL:(NSString *)RTMRouterURL callback:(void (^)(NSDictionary *dataDictionary, NSError *error))callback
{
    ...
    [[TGBPaasClient sharedInstance] getObject:RTMRouterURL withParameters:parameters block:^(id _Nullable object, NSError * _Nullable error) {
        if (error) {
            callback(nil, error);
        } else {
...
- (void)customAppServerURL:(NSString *)URLString key:(RouterKey)key
{
    ...
    TGBNetworkStatistics.sharedInstance.ignoreAlwaysCollectIfCustomedService = true;
}
...
static RouterKey serverKeyForPath(NSString *path)
{
    ...
    } else if ([path isEqualToString:[TGBRouter RTMRouterPath]]) {
