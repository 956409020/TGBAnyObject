...
@implementation TGBRequestManager {
...
+ (NSDictionary *)commonOpForKey:(NSString *)key objects:(NSArray *)objects op:(NSString *)op {
    ...
    for(TGBObject * obj in objects) {
        NSDictionary * dict = [TGBObjectUtils dictionaryFromObject:obj];
...
-(NSDictionary *)jsonForSetWithIgnoreTGBObject:(BOOL)ignoreTGBObject {
    ...
    for(NSString * key in setDict) {
        ...
        if (ignoreTGBObject &&
            [object isKindOfClass:[TGBObject class]] &&
            ![object hasValidObjectId]) {
            continue;
        }

        NSDictionary * jsonDict = [TGBObjectUtils dictionaryFromObject:object];
        dict[key] = jsonDict;
    }
...
- (NSDictionary *)jsonForOp:(AVOp)op {
    if (op == SET) {
        return [self jsonForSetWithIgnoreTGBObject:YES];
    } else {
        ...
        [requestDict enumerateKeysAndObjectsUsingBlock:^(NSString *key, id object, BOOL *stop) {
            ...
            if (op == INC) {
                ...
                jsonDict = [TGBRequestManager incOpForKey:key value:value];
            } else if (op == UNSET) {
                jsonDict = [TGBRequestManager unsetOpForKey:key];
            } else {
...
-(NSMutableDictionary *)initialSetDict {
    ...
    [self synchronize:^{
        NSDictionary * dict = [self jsonForSetWithIgnoreTGBObject:YES];
