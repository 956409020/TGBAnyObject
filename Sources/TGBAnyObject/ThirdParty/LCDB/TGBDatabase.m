...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    TGBDatabase *self = (__bridge TGBDatabase*)f;
...
- (void)closeOpenResultSets {
    ...
    for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
        TGBResultSet *rs = (TGBResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
...
- (void)resultSetDidClose:(TGBResultSet *)resultSet {
...
- (BOOL)goodConnection {
    ...
    TGBResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (NSError*)errorWithMessage:(NSString*)message {
    ...
    return [NSError errorWithDomain:@"TGBDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];    
}
...
- (TGBResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
...
//        NSLog(@"Error: the bind count is not correct for the # of variables (executeQuery)");
    ...
    rs = [TGBResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (TGBResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (TGBResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (TGBResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeQuery:sql withArgumentsInArray:arguments orDictionary:nil orVAList:nil];
}

- (TGBResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
