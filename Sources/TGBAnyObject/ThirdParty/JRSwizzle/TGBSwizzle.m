...
@implementation NSObject (TGBSwizzle)
...
+ (BOOL)lc_swizzleClassMethod:(SEL)origSel_ withClassMethod:(SEL)altSel_ error:(NSError**)error_ {
	return [GetClass((id)self) lc_swizzleMethod:origSel_ withMethod:altSel_ error:error_];
}
...
+ (NSInvocation*)lc_swizzleMethod:(SEL)origSel withBlock:(id)block error:(NSError**)error {
    ...
    [self lc_swizzleMethod:origSel withMethod:blockSel error:nil];

    return origInvocation;
}

+ (NSInvocation*)lc_swizzleClassMethod:(SEL)origSel withBlock:(id)block error:(NSError**)error {
    NSInvocation *invocation = [GetClass((id)self) lc_swizzleMethod:origSel withBlock:block error:error];
