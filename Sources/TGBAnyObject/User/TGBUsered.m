...
@implementation TGBUseredAuthDataLoginOption

@end

@implementation  TGBUsered
...
{
    return [TGBUsered userTag];
}

+ (void)changeCurrentUser:(TGBUsered *)newUser save:(BOOL)save
{
    if (newUser && save) {
        NSMutableDictionary * json = [newUser userDictionaryForCache];
        [json removeObjectForKey:passwordTag];
        [TGBPersistenceUtils saveJSON:json toPath:[TGBPersistenceUtils currentUserArchivePath]];
        [TGBPersistenceUtils saveJSON:@{@"class": NSStringFromClass([newUser class])}
                              toPath:[TGBPersistenceUtils currentUserClassArchivePath]];
    } else if (save) {
        [TGBPersistenceUtils removeFile:[TGBPersistenceUtils currentUserArchivePath]];
        [TGBPersistenceUtils removeFile:[TGBPersistenceUtils currentUserClassArchivePath]];
    }
    [TGBPaasClient sharedInstance].currentUser = newUser;
}

+ (instancetype)currentUser
{
    TGBUsered *user = [TGBPaasClient sharedInstance].currentUser;
    if (user) {
        return user;
    } else if ([TGBPersistenceUtils fileExist:[TGBPersistenceUtils currentUserArchivePath]]) {
        NSMutableDictionary *userDict = [NSMutableDictionary dictionaryWithDictionary:[TGBPersistenceUtils getJSONFromPath:[TGBPersistenceUtils currentUserArchivePath]]];
        if (userDict) {
            if ([TGBPersistenceUtils fileExist:[TGBPersistenceUtils currentUserClassArchivePath]]) {
                NSDictionary *classDict = [TGBPersistenceUtils getJSONFromPath:[TGBPersistenceUtils currentUserClassArchivePath]];
                user = [NSClassFromString(classDict[@"class"]) user];
            } else {
                user = [self userOrSubclassUser];
            }
            
            [TGBObjectUtils copyDictionary:userDict toObject:user];
            [TGBPaasClient sharedInstance].currentUser = user;
            return user;
        }
    }
    if (!enableAutomatic) {
    ...
    TGBUsered *newUser = [self userOrSubclassUser];
...
- (void)isAuthenticatedWithSessionToken:(NSString *)sessionToken callback:(AVBooleanResultBlock)callback {
    if (sessionToken == nil) {
        [TGBUtils callBooleanResultBlock:callback error:LCErrorInternal(@"sessionToken is nil")];
        return;
    }
    
    [[TGBPaasClient sharedInstance] getObject:[NSString stringWithFormat:@"%@/%@", [[self class] endPoint], @"me"] withParameters:@{@"session_token": sessionToken} block:^(id object, NSError *error) {
...
- (NSArray<TGBRole *> *)getRoles:(NSError * _Nullable __autoreleasing *)error {
    TGBQuery *query = [TGBRelation reverseQuery:@"_Role" relationKey:@"users" childObject:self];
    return [query findObjects:error];
}

- (NSArray<TGBRole *> *)getRolesAndThrowsWithError:(NSError * _Nullable __autoreleasing *)error {
    return [self getRoles:error];
}

- (void)getRolesInBackgroundWithBlock:(void (^)(NSArray<TGBRole *> * _Nullable, NSError * _Nullable))block {
    [TGBUtils asynchronizeTask:^{
        NSError *error = nil;
        NSArray<TGBRole *> *result = [self getRoles:&error];
        [TGBUtils callArrayResultBlock:block array:result error:error];
    }];
}

+ (instancetype)user
{
    TGBUsered *u = [[[self class] alloc] initWithClassName:[[self class] userTag]];
    return u;
}
...
+(void)requestEmailVerify:(NSString*)email withBlock:(AVBooleanResultBlock)block{
    ...
    [[TGBPaasClient sharedInstance] postObject:@"requestEmailVerify" withParameters:@{@"email":email} block:^(id object, NSError *error) {
        [TGBUtils callBooleanResultBlock:block error:error];
    }];
}
...
+ (void)requestVerificationCodeForPhoneNumber:(NSString *)phoneNumber
                                      options:(TGBUseredShortMessageRequestOptions *)options
                                     callback:(AVBooleanResultBlock)callback
{
    ...
    [[TGBPaasClient sharedInstance] postObject:@"requestMobilePhoneVerify" withParameters:parameters block:^(id object, NSError *error) {
        [TGBUtils callBooleanResultBlock:callback error:error];
    }];
}
...
+(void)verifyMobilePhone:(NSString *)code withBlock:(AVBooleanResultBlock)block {
    ...
    [[TGBPaasClient sharedInstance] getObject:path withParameters:nil block:^(id object, NSError *error) {
        if (!error) {
            [[TGBUsered currentUser] setMobilePhoneVerified:YES];
        } else {
            [[TGBUsered currentUser] setMobilePhoneVerified:NO];
        }
        [self changeCurrentUser:[TGBUsered currentUser] save:YES];
        [TGBUtils callBooleanResultBlock:block error:error];
    }];
...
- (void)updatePassword:(NSString *)oldPassword newPassword:(NSString *)newPassword block:(AVIdResultBlock)block {
    if (self.isAuthDataExistInMemory && oldPassword && newPassword) {
        ...
        [[TGBPaasClient sharedInstance] putObject:path withParameters:params sessionToken:self.sessionToken block:^(id object, NSError *error) {
            if (!error) {
                ...
                if (self == [TGBUsered currentUser]) {
                    [TGBUsered changeCurrentUser:self save:YES];
                }
            }
        ...
        [TGBUtils callIdResultBlock:block object:nil error:error];
    }
}
...
- (void)refreshSessionTokenWithBlock:(AVBooleanResultBlock)block {
    ...
    TGBPaasClient *HTTPClient = [TGBPaasClient sharedInstance];
    ...
    [HTTPClient performRequest:request
                       success:^(NSHTTPURLResponse *response, id result) {
                           ...
                           self.updatedAt = [TGBObjectUtils dateFromString:result[@"updatedAt"]];

                           if ([self isEqual:[TGBUsered currentUser]]) {
                               [TGBUsered changeCurrentUser:self save:YES];
                           }
...
+ (instancetype)logInWithUsername:(NSString *)username
                     password:(NSString *)password
                        error:(NSError **)error
{
    __block TGBUsered * resultUser = nil;
    [[self class] logInWithUsername:username password:password block:^(TGBUsered *user, NSError *error) {
        resultUser = user;
                             ...
                             selector:(SEL)selector
{
    [[self class] logInWithUsernameInBackground:username
                                       password:password
                                          block:^(TGBUsered *user, NSError *error) {
...
+ (BOOL)logInWithUsername:(NSString *)username
    ...
    [[TGBPaasClient sharedInstance] postObject:@"login" withParameters:parameters block:^(id object, NSError *error) {
        TGBUsered * user = nil;
        if (error == nil)
        {
            user = [self userOrSubclassUser];
...
{
    __block TGBUsered * resultUser = nil;
    [self logInWithMobilePhoneNumber:phoneNumber password:password block:^(TGBUsered *user, NSError *error) {
        resultUser = user;
    } waitUntilDone:YES error:error];
    return resultUser;
}
...
+ (void)becomeWithSessionTokenInBackground:(NSString *)sessionToken block:(TGBUseredResultBlock)block {
    [self internalBecomeWithSessionTokenInBackground:sessionToken block:^(TGBUsered *user, NSError *error) {
        [TGBUtils callUserResultBlock:block user:user error:error];
    }];
}

+ (void)internalBecomeWithSessionTokenInBackground:(NSString *)sessionToken block:(TGBUseredResultBlock)block {
    if (sessionToken == nil) {
        if (block) {
    ...
    [[TGBPaasClient sharedInstance] getObject:[NSString stringWithFormat:@"%@/%@", [self endPoint], @"me"] withParameters:@{@"session_token": sessionToken} block:^(id object, NSError *error) {
        TGBUsered *user;
        if (!error) {
            user = [self userOrSubclassUser];
...
+ (instancetype)becomeWithSessionToken:(NSString *)sessionToken error:(NSError * __autoreleasing *)error {
    ...
    __block TGBUsered *user;
    [self internalBecomeWithSessionTokenInBackground:sessionToken block:^(TGBUsered *theUser, NSError *theError) {
        user = theUser;
        if (error) {
            *error = theError;
        }
                              ...
                              callback:(AVBooleanResultBlock)callback
{
    ...
    [[TGBPaasClient sharedInstance] postObject:@"requestLoginSmsCode" withParameters:parameters block:^(id object, NSError *error) {
        [TGBUtils callBooleanResultBlock:callback error:error];
    }];
}
...
{
    __block TGBUsered * resultUser = nil;
    [self logInWithMobilePhoneNumber:phoneNumber smsCode:code block:^(TGBUsered *user, NSError *error) {
        resultUser = user;
    } waitUntilDone:YES error:error];
    return resultUser;
}
...
+ (instancetype)signUpOrLoginWithMobilePhoneNumber:(NSString *)phoneNumber
                                           smsCode:(NSString *)code
                                             error:(NSError **)error {
    __block TGBUsered * resultUser = nil;
    [self signUpOrLoginWithMobilePhoneNumber:phoneNumber smsCode:code block:^(TGBUsered *user, NSError *error) {
        resultUser = user;
    } waitUntilDone:YES error:error];
    return resultUser;
}
...
+ (BOOL)signUpOrLoginWithMobilePhoneNumber:(NSString *)phoneNumber
    ...
    NSDictionary * parameters = @{mobilePhoneNumberTag: phoneNumber, smsCodeTag:smsCode};
    [[TGBPaasClient sharedInstance] postObject:@"usersByMobilePhone" withParameters:parameters block:^(id object, NSError *error) {
        TGBUsered * user = nil;
        if (error == nil)
        {
            user = [self userOrSubclassUser];
...
{    
    ...
    [[TGBPaasClient sharedInstance] postObject:@"usersByMobilePhone" withParameters:parameters block:^(id object, NSError *error) {
        if (error) {
            [TGBUtils callUserResultBlock:block user:nil error:error];
            return;
        }
        TGBUsered *user = [self userOrSubclassUser];
        [self configAndChangeCurrentUserWithUser:user object:object];
        [TGBUtils callUserResultBlock:block user:user error:nil];
    }];
}
                                           ...
                                           block:(AVBooleanResultBlock)block
{
    ...
    [[TGBPaasClient sharedInstance] postObject:@"requestPasswordReset" withParameters:parameters block:^(id object, NSError *error) {
        if (block) {
                                      ...
                                      callback:(AVBooleanResultBlock)callback
{
    ...
    [[TGBPaasClient sharedInstance] postObject:@"requestPasswordResetBySmsCode" withParameters:parameters block:^(id object, NSError *error) {
        [TGBUtils callBooleanResultBlock:callback error:error];
    }];
}
...
+(void)resetPasswordWithSmsCode:(NSString *)code
    ...
    [[TGBPaasClient sharedInstance] putObject:path withParameters:@{ @"password" : password } sessionToken:nil block:^(id object, NSError *error) {
        [TGBUtils callBooleanResultBlock:block error:error];
    }];
}
...
+ (TGBQuery *)query
{
    TGBQuery *query = [[TGBQuery alloc] initWithClassName:[[self class] userTag]];
    return query;
}
                 ...
                 callback:(void (^)(BOOL, NSError * _Nullable))callback
{
    ...
    [TGBPaasClient.sharedInstance postObject:path withParameters:parameters block:^(id object, NSError *error) {
        ...
        [TGBObjectUtils copyDictionary:dic toObject:self];
                     ...
                     callback:(void (^)(BOOL, NSError * _Nullable))callback
{
    ...
    [TGBPaasClient.sharedInstance putObject:path withParameters:parameters sessionToken:sessionToken block:^(id object, NSError *error) {
...
+ (void)loginAnonymouslyWithCallback:(void (^)(TGBUsered * _Nullable user, NSError * _Nullable error))callback
{
    ...
    [TGBPaasClient.sharedInstance postObject:@"users" withParameters:parameters block:^(id object, NSError *error) {
        ...
        TGBUsered *user = [TGBUsered userOrSubclassUser];
...
#pragma mark - Override from TGBObject
...
+ (TGBUsered *)userOrSubclassUser {
    return (TGBUsered *)[TGBObjectUtils TGBObjectForClass:[TGBUsered userTag]];
}
                                    ...
                                    object:(id)object
{
    ...
    [TGBObjectUtils copyDictionary:dic toObject:user];
...
@implementation TGBUsered (Friendship)

+(TGBQuery*)followerQuery:(NSString*)userObjectId{
    TGBFriendQuery *query=[TGBFriendQuery queryWithClassName:@"_Follower"];
...
+(TGBQuery*)followeeQuery:(NSString*)userObjectId{
    TGBFriendQuery *query=[TGBFriendQuery queryWithClassName:@"_Followee"];
...
-(TGBQuery*)followeeQuery{
    return [TGBUsered followeeQuery:self.objectId];
}

-(TGBQuery*)followerQuery{
    return [TGBUsered followerQuery:self.objectId];
}
...
-(void)follow:(NSString*)userId userDictionary:(NSDictionary *)dictionary andCallback:(AVBooleanResultBlock)callback{
    ...
    NSDictionary *dict = [TGBObjectUtils dictionaryFromObject:dictionary];
    NSString *path=[NSString stringWithFormat:@"users/self/friendship/%@",userId];
    
    [[TGBPaasClient sharedInstance] postObject:path withParameters:dict block:^(NSDictionary *object, NSError *error) {
        [TGBUtils callBooleanResultBlock:callback error:error];
    }];
}
...
-(void)unfollow:(NSString *)userId andCallback:(AVBooleanResultBlock)callback{
    ...
    [[TGBPaasClient sharedInstance] deleteObject:path withParameters:nil block:^(NSDictionary *object, NSError *error) {
        [TGBUtils callBooleanResultBlock:callback error:error];
    }];
}
...
-(void)getFollowers:(AVArrayResultBlock)callback{
    ...
    TGBQuery *query= [TGBUsered followerQuery:self.objectId];
...
-(void)getFollowees:(AVArrayResultBlock)callback{
    ...
    TGBQuery *query= [TGBUsered followeeQuery:self.objectId];
...
-(void)getFollowersAndFollowees:(AVDictionaryResultBlock)callback{
    ...
    [[TGBPaasClient sharedInstance] getObject:path withParameters:nil block:^(NSDictionary *object, NSError *error) {
        if (error==nil) {
            ...
            @try {
                ...
                result=[TGBObjectUtils arrayFromArray:orig];
            ...
            @finally {
                [TGBUtils callIdResultBlock:callback object:dict error:error];
            }
        } else {
...
@implementation TGBUseredShortMessageRequestOptions
