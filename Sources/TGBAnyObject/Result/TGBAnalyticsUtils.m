...
@implementation TGBAnalyticsUtils
...
+(BOOL)isWiFiConnection {
    return [[TGBAnalyticsUtils connectionType] isEqualToString:WiFiType];
}

+(NSString *)connectionType
{
    NSString *type = @"";
    TGBNetworkReachabilityManager *reachabilityManager = [TGBNetworkReachabilityManager sharedManager];
...
+(NSMutableDictionary *)deviceInfo
{
...
#if !TARGET_OS_WATCH
#if defined(__IPHONE_OS_VERSION_MIN_REQUIRED)
        NSString *deviceId = [TGBAnalyticsUtils deviceId];
        if (deviceId) { staticDic[@"device_id"] = deviceId; }
#endif
#endif
        
        staticDic[@"is_jailbroken"] = @([TGBAnalyticsUtils isJailbroken]);
        
        NSString *deviceModel = [TGBAnalyticsUtils deviceModel];
        if (deviceModel) { staticDic[@"device_model"] = deviceModel; }
        
        NSString *resolution = [TGBAnalyticsUtils screenResolution];
        if (resolution) { staticDic[@"resolution"] = resolution; }
        
        NSString *osVersion = [TGBAnalyticsUtils systemVersion];
        if (osVersion) { staticDic[@"os_version"] = osVersion; }
        
        NSString *language = [TGBAnalyticsUtils language];
        if (language) { staticDic[@"language"] = language; }
        
        NSString *timezone = [TGBAnalyticsUtils timezone];
        if (timezone) { staticDic[@"timezone"] = timezone; }
        
        NSString *buildVersion = [TGBAnalyticsUtils buildVersion];
        if (buildVersion) { staticDic[@"sv"] = buildVersion; }

        NSString *appVersion = [TGBAnalyticsUtils appVersion];
        if (appVersion) { staticDic[@"app_version"] = appVersion; }
        
        NSString *packageName = [TGBAnalyticsUtils packageName];
        if (packageName) { staticDic[@"package_name"] = packageName; }
        
        NSString *displayName = [TGBAnalyticsUtils displayName];
...
#if !TARGET_OS_WATCH
    NSString *connectionType = [TGBAnalyticsUtils connectionType];
    if (connectionType) { dynamicDic[@"access"] = connectionType; }
    
    NSString *carrier = [TGBAnalyticsUtils carrier];
    ...
    TGBUsered *currentUser = [TGBUsered currentUser];
    ...
    TGBInstallation *currentInstallation = [TGBInstallation defaultInstallation];
...
+(BOOL)inDebug {
    return [TGBAnalyticsUtils inSimulator];
}
