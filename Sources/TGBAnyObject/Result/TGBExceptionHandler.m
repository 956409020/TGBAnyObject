...
void globalHandleException(NSException *exception)
{
	...
	[[[TGBExceptionHandler alloc] init]
...
void globalSignalHandler(int signal)
{
	...
	NSArray *callStack = [TGBExceptionHandler backtrace];
	[userInfo setObject:callStack forKey:AVOS_UncaughtExceptionHandlerAddressesKey];
...
@end
@implementation TGBExceptionHandler
...
- (void)handleException:(NSException *)exception
{
    [[TGBAnalyticsImpl sharedInstance] addException:exception];
	
    //需要让开发者设置忽略crash吗? 我们可以提醒用户app已经不稳定 但这对开发者来说比退出要好
    
    
    if ([TGBAnalyticsImpl sharedInstance].enableIgnoreCrash) {
        self.holdException=YES;
        NSString *title=nil,*msg=nil,*quit=nil,*ctn=nil;
        NSDictionary *ignoreCrashAlertStrings=[TGBAnalyticsImpl sharedInstance].ignoreCrashAlertStrings;
        if ([TGBAnalyticsImpl sharedInstance].ignoreCrashAlertStrings) {
