...
@implementation TGBAnalyticsImpl

@synthesize reportPolicy =  _reportPolicy;

+(TGBAnalyticsImpl *)sharedInstance
{
    static dispatch_once_t once;
    static TGBAnalyticsImpl * sharedInstance;
    dispatch_once(&once, ^{
...
-(void)_enableCrashReportRunloop{
    [TGBExceptionHandler installAVOSUncaughtExceptionHandler];
}
...
-(void)setEnableCrashReport:(BOOL)enabled completion:(void (^)(void))completion {
    ...
    if (self.enableCrashReport) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [TGBExceptionHandler uninstallAVOSUncaughtExceptionHandler];
            if (completion) {
                completion();
            }
        });
...
-(void)addException:(NSException *)exception{
    ...
    NSMutableDictionary * dict = [TGBAnalyticsUtils deviceInfo];
    
    [dict addEntriesFromDictionary:@{
                                     @"type": [exception name],
                                     @"reason": [exception reason],
                                     @"stack_trace": strace,
                                     @"time":  @([TGBAnalyticsUtils currentTimestamp]),
                                     }];
    ...
    [dict setObject:[TGBExceptionHandler appBuildUUID] forKey:@"build_uuid"];
    
    [[TGBPaasClient sharedInstance] postObject:[[[self class] myObjectPath] stringByAppendingPathComponent:@"crash"]
                               withParameters:dict
                                   eventually:YES
...
-(TGBAnalyticsSession *)currentSession
{
    for(TGBAnalyticsSession * session in [self.sessions copy]) {
...
{
    NSTimeInterval current = [TGBAnalyticsUtils currentTimestamp];
...
- (void)av_applicationWillResignActive:(id)sender {
    ...
    // application paused
    self.pauseTimeStamp = [TGBAnalyticsUtils currentTimestamp];
...
-(TGBAnalyticsSession *)createSession
{
    TGBAnalyticsSession * session = [[TGBAnalyticsSession alloc] init];
    return session;
}

-(void)beginSession
{
    TGBAnalyticsSession * session = [self currentSession];
    if (session == nil) {
...
-(NSDictionary *)launchDictionary
{
    ...
    NSDictionary *dict=@{kAVSessionIdTag: session.sessionId, kAVDateTag: @([session.duratioTGBpl createTimeStampInMilliSeconds])};
    return dict;
}
...
{
    return [[TGBRouter sharedInstance] appURLForPath:@"stats" appID:[TGBKKit getApplicationId]];
}
...
- (void)sendOneSession:(NSDictionary *)session {
    [[TGBPaasClient sharedInstance] postObject:[[[self class] myObjectPath] stringByAppendingString:@"/collect"]
...
-(void)postRecording {
    if (self.reportPolicy ==  AV_REALTIME) {
        ...
        if ([TGBAnalyticsUtils isWiFiConnection]) {
            [self sendWithBatch];
        }
