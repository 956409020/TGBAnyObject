...
typedef NSString TGBStatusType;

@class TGBStatus, TGBStatusQuery;

NS_ASSUME_NONNULL_BEGIN

extern NSString * const kTGBStatusTypeTimeline;
extern NSString * const kTGBStatusTypePrivateMessage;

typedef void (^TGBStatusResultBlock)(TGBStatus * _Nullable status, NSError * _Nullable error);

/**
 *  发送和获取状态更新和消息
 */
@interface TGBStatus : NSObject
...
/**
 *  状态的发出"人",可以是TGBUsered 也可以是任意的TGBObject,也可能是nil
 */
@property (nonatomic, strong, nullable) TGBObject *source;

/**
 *  状态类型,默认是kTGBStatusTypeTimeline, 可以是任意自定义字符串
 */
@property (nonatomic, copy) TGBStatusType *type;
...
/**
 *  获取当前用户接收到的状态
 *  @param type     状态类型,默认是kTGBStatusTypeTimeline, 可以是任意自定义字符串
 *  @param skip     跳过条数
 *  @param limit    需要返回的条数 默认`100`，最大`100`
 *  @param callback 回调结果
 */
+(void)getStatusesWithType:(TGBStatusType*)type skip:(NSUInteger)skip limit:(NSUInteger)limit andCallback:(AVArrayResultBlock)callback AV_DEPRECATED("2.3.2以后不再需要，请使用inboxQuery类方法");
 ...
 *  @param callback 回调结果
 */
+(void) getStatusesFromCurrentUserWithType:(TGBStatusType*)type skip:(NSUInteger)skip limit:(NSUInteger)limit andCallback:(AVArrayResultBlock)callback AV_DEPRECATED("2.3.2以后不再需要，请使用statusQuery类方法");
 ...
 *  用户好友关系
 */
@interface TGBUsered(Friendship)
...
/**
 *  通过ID来关注其他用户
 *  @warning 如果需要被关注者收到消息 需要手动给他发送一条TGBStatus.
 ...
 *  查询TGBStatus
 */
@interface TGBStatusQuery : TGBQuery
/**
 ...
 */
@property(nonatomic, strong, nullable) TGBObject *owner;

/**
 *  设置查询的Inbox的类型
 */
@property(nonatomic, copy, nullable) TGBStatusType *inboxType;
