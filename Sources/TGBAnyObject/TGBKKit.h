...
//  ClourdOC
//
//  Created by TGB on 2018/12/30.
//  Copyright © 2018年 TGB. All rights reserved.
//
 ...
 *  @param enabled 开启
 *  @attention 该方法并不会修改任何TGBQuery的缓存策略，缓存策略以当前TGBQuery的设置为准。该方法仅在进行网络请求时生效。如果想发挥该函数的最大作用，建议在查询时，将缓存策略选择为kAVCachePolicyNetworkOnly
 */
...
/**
 ...
 @param deviceToken Device token issued by APNs.
 @param block Constructing block of [TGBInstallation defaultInstallation].
 */
+ (void)handleRemoteNotificationsWithDeviceToken:(NSData *)deviceToken
               constructingInstallationWithBlock:(void(^ _Nullable)(TGBInstallation *currentInstallation))block;
...
+ (void)handleRemoteNotificationsWithDeviceToken:(NSData *)deviceToken
                                          teamId:(NSString * _Nullable)teamId
               constructingInstallationWithBlock:(void(^ _Nullable)(TGBInstallation *))block;
...
@interface TGBKKit (AVDeprecated)
...
+(void)requestSmsCodeWithPhoneNumber:(NSString *)phoneNumber
                            callback:(AVBooleanResultBlock)callback AV_DEPRECATED("Deprecated in AVOSCloud SDK 4.5.0. Use +[TGBSMS requestShortMessageForPhoneNumber:options:callback:] instead.");
