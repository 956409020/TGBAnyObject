...
static NSString * TGBFile_CustomPersistentCacheDirectory = nil;

static NSString * TGBFile_PersistentCacheDirectory()
{
    return TGBFile_CustomPersistentCacheDirectory ?: [TGBPersistenceUtils homeDirectoryLibraryCachesLeanCloudCachesFiles];
}

static NSString * TGBFile_CompactUUID()
{
    return [TGBUtils generateCompactUUID];
}

static NSString * TGBFile_ObjectPath(NSString *objectId)
{
    return (objectId && objectId.length > 0) ? [@"classes/_file" stringByAppendingPathComponent:objectId] : nil;
}

@implementation TGBFile {
...
{
    return [[TGBFile alloc] initWithData:data name:nil];
}

+ (instancetype)fileWithData:(NSData *)data name:(NSString *)name
{
    return [[TGBFile alloc] initWithData:data name:name];
}

+ (instancetype)fileWithLocalPath:(NSString *)localPath
                            error:(NSError * __autoreleasing *)error
{
    return [[TGBFile alloc] initWithLocalPath:localPath error:error];
}

+ (instancetype)fileWithRemoteURL:(NSURL *)remoteURL
{
    return [[TGBFile alloc] initWithRemoteURL:remoteURL];
}

+ (instancetype)fileWithTGBObject:(TGBObject *)TGBObject
{
    return [[TGBFile alloc] initWithRawJSONData:[TGBObject dictionaryForObject]];
}
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _rawJSONData = [NSMutableDictionary dictionaryWithObject:TGBFile.className forKey:kLCFile___type];
    }
                        ...
                        name:(NSString *)name
{
    ...
    if (self) {
        ...
        _rawJSONData[kLCFile_name] = (name && name.length > 0) ? name : TGBFile_CompactUUID();
        
        _rawJSONData[kLCFile_mime_type] = ({
            
            NSString *mimeType = nil;
            if (name && name.length > 0) {
                mimeType = [TGBUtils MIMEType:name];
            }
            if (!mimeType && data.length > 0) {
                mimeType = [TGBUtils contentTypeForImageData:data];
            }
        ...
        _rawJSONData[kLCFile_metaData] = ({
            ...
            NSString *objectId = TGBPaasClient.sharedInstance.currentUser.objectId;
        ...
        _ACL = ({
            ...
            TGBACL *acl = TGBPaasClient.sharedInstance.updatedDefaultACL;
            if (acl) {
                _rawJSONData[ACLTag] = [TGBObjectUtils dictionaryFromACL:acl];
            }
            acl;
        });
    }
                            ...
                            error:(NSError * __autoreleasing *)error
{
    ...
    if (self) {
        ...
        NSString *name = ({
            ...
            (lastPathComponent && lastPathComponent.length > 0) ? lastPathComponent : TGBFile_CompactUUID();
        });
        ...
        _rawJSONData[kLCFile_mime_type] = ({
            ...
            if (!mimeType && localPath.length > 0) {
                mimeType = [TGBUtils MIMETypeFromPath:localPath];
            }
            mimeType ?: @"application/octet-stream";
        });
    ...
    if (self) {
        ...
        _rawJSONData[kLCFile_mime_type] = ({
            ...
            if (absoluteString && absoluteString.length > 0) {
                mimeType = [TGBUtils MIMEType:absoluteString];
            }
            mimeType ?: @"application/octet-stream";
        });
...
- (instancetype)initWithRawJSONData:(NSMutableDictionary *)rawJSONData
{
    ...
    if (self) {
        ...
        _ACL = TGBPaasClient.sharedInstance.updatedDefaultACL;
    }
...
{
    NSDictionary *ACLDic = [TGBObjectUtils dictionaryFromACL:ACL];
...
- (NSString *)objectId
{
    ...
    [self internalSyncLock:^{
        ...
        objectId = [TGBFile decodingObjectIdFromDic:_rawJSONData];
    }];
...
- (NSDictionary *)metaData
{
    ...
    [self internalSyncLock:^{
        ...
        metaData = [TGBFile decodingMetaDataFromDic:_rawJSONData];
    }];
...
- (void)setOwnerId:(NSString *)ownerId
{
    [self internalSyncLock:^{
        ...
        NSMutableDictionary *metaData = [[TGBFile decodingMetaDataFromDic:_rawJSONData] mutableCopy];
...
{
    [self uploadWithOption:TGBFileUploadOptionCachingData
                  progress:nil
...
- (void)uploadWithOption:(TGBFileUploadOption)uploadOption
       ...
       completionHandler:(void (^)(BOOL, NSError * _Nullable))completionHandler
{
    ...
    if (self->_data || self->_localPath) {
        ...
        [self uploadLocalDataWithData:data localPath:localPath progress:progress completionHandler:^(BOOL succeeded, NSError *error) {
            ...
            TGBFileUploadOption uploadOption = ({
                
                __block TGBFileUploadOption uploadOption = 0;
                [self internalSyncLock:^{
            ...
            if (succeeded && !(uploadOption & TGBFileUploadOptionIgnoringCachingData)) {
              ...
              completionHandler:(void (^)(BOOL succeeded, NSError *error))completionHandler
{
    NSMutableDictionary *mutableParameters = ({
        ...
        mutableDic[kLCFile_key] = ({
            NSString *key = TGBFile_CompactUUID();
            if (_pathExtension) {
    ...
    [self getFileTokensWithParameters:parameters callback:^(LCFileTokens *fileTokens, NSError *error) {
        ...
        NSURLSessionUploadTask *task = ({
            ...
            void(^completionHandler_block)(BOOL success, NSError *error) = ^(BOOL success, NSError *error) {
                ...
                void(^fileCallback_block)(BOOL succeeded) = ^(BOOL succeeded) {
                    ...
                    if (fileTokens && fileTokens.token) {
                        [TGBPaasClient.sharedInstance postObject:@"fileCallback"
            ...
            if (data) {
                task = [TGBFileTaskManager.sharedInstance uploadTaskWithData:data
                ...
                task = [TGBFileTaskManager.sharedInstance uploadTaskWithLocalPath:localPath
                                                                      fileTokens:fileTokens
                                                                  fileParameters:parameters
                                                                uploadingHeaders:uploadingHeaders
                                                                        progress:progress
...
- (void)uploadRemoteURLWithCompletionHandler:(void (^)(BOOL succeeded, NSError *error))completionHandler
{
    ...
    [TGBPaasClient.sharedInstance postObject:@"files" withParameters:mutableParameters.copy block:^(id object, NSError *error) {
...
{
    [TGBPaasClient.sharedInstance postObject:@"fileTokens" withParameters:parameters block:^(id _Nullable object, NSError * _Nullable error) {
...
{
    [self downloadWithOption:TGBFileDownloadOptionCachedData
                    progress:nil
           completionHandler:completionHandler];
}
...
- (void)downloadWithOption:(TGBFileDownloadOption)downloadOption
         ...
         completionHandler:(void (^)(NSURL * _Nullable, NSError * _Nullable))completionHandler
{
    ...
    if (!(downloadOption & TGBFileDownloadOptionIgnoringCachedData)) {
    ...
    NSURLSessionDownloadTask *task = ({
        ...
        [TGBFileTaskManager.sharedInstance downloadTaskWithURLString:URLString destination:^NSURL *(NSURL *targetPath, NSURLResponse *response) {
...
{
    TGBFile_CustomPersistentCacheDirectory = directory;
}
...
{
    NSString *directoryPath = TGBFile_PersistentCacheDirectory();
    if (!directoryPath) {
        return;
    }
...
- (NSString *)persistentCachePathThrowError:(NSError * __autoreleasing *)error
{
    ...
    NSString *directory = TGBFile_PersistentCacheDirectory();
...
{
    NSString *objectPath = TGBFile_ObjectPath([self objectId]);
    ...
    [[TGBPaasClient sharedInstance] deleteObject:objectPath withParameters:nil block:^(id _Nullable object, NSError * _Nullable error) {
...
+ (void)deleteWithFiles:(NSArray<TGBFile *> *)files
      completionHandler:(void (^)(BOOL, NSError * _Nullable))completionHandler
{
    ...
    for (TGBFile *file in files) {
        ...
        NSMutableDictionary *request = [TGBPaasClient batchMethod:@"DELETE"
                                                            path:objectPath
                                                            body:nil
                                                      parameters:nil];
    ...
    [[TGBPaasClient sharedInstance] postBatchObject:requests block:^(NSArray * _Nullable objects, NSError * _Nullable error) {
...
+ (void)getFileWithObjectId:(NSString *)objectId
          completionHandler:(void (^)(TGBFile *file, NSError *error))completionHandler
{
    NSString *objectPath = TGBFile_ObjectPath(objectId);
    ...
    [[TGBPaasClient sharedInstance] getObject:objectPath withParameters:nil block:^(id object, NSError* error) {
        ...
        TGBFile *file = [[TGBFile alloc] initWithRawJSONData:dic.mutableCopy];
          ...
          withBlock:(AVImageResultBlock)block
{
    ...
    [[TGBFileTaskManager sharedInstance] getThumbnailWithURLString:url completionHandler:^(id thumbnail, NSError *error) {
