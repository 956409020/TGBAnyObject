...
@implementation TGBFileTaskManager {
    
    TGBURLSessionManager *_URLSessionManager;
    
    LCHTTPRequestSerializer *_requestSerializer;
    
    TGBURLSessionManager *_thumbnailSessionManager;
}

+ (instancetype)sharedInstance
{
    static TGBFileTaskManager *instance = nil;
    
    static dispatch_once_t onceToken;
    
    dispatch_once(&onceToken, ^{
        
        instance = [[TGBFileTaskManager alloc] init];
    });
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _URLSessionManager = ({
            TGBURLSessionManager *manager = [[TGBURLSessionManager alloc] init];
            manager.responseSerializer = [LCHTTPResponseSerializer serializer];
            manager.completionGroup = group;
            manager.completionQueue = dispatch_queue_create("queue.serial.TGBFileTaskManager", DISPATCH_QUEUE_CONCURRENT);
            manager;
        });
        ...
        _thumbnailSessionManager = ({
            ...
            manager.completionGroup = group;
            manager.completionQueue = dispatch_queue_create("queue.concurrent.TGBFileTaskManager.thumbnail", DISPATCH_QUEUE_CONCURRENT);
            manager;
        });
    }
