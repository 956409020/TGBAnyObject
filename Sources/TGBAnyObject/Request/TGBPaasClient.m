...
@interface TGBPaasClient()

@property (nonatomic, strong) TGBURLSessionManager *sessionManager;
...
@implementation TGBPaasClient

+(TGBPaasClient *)sharedInstance
{
    static dispatch_once_t once;
    static TGBPaasClient * sharedInstance;
    dispatch_once(&once, ^{
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _sessionManager = ({
            ...
            TGBURLSessionManager *manager = [[TGBURLSessionManager alloc] initWithSessionConfiguration:configuration];
            manager.completionQueue = _completionQueue;

            if ([TGBKKit isSSLPinningEnabled]) {
                ...
                [manager setSessionDidReceiveAuthenticationChallengeBlock:
                 ...
                 {
                     [[TGBSSLChallenger sharedInstance] acceptChallenge:challenge];
...
-(void)clearLastModifyCache {
    if (self.lastModify.count) {
        for (NSString *key in self.lastModify) {
            [[TGBCacheManager sharedInstance] clearCacheForMD5Key:key];
        }
                         ...
                         parameters:(NSDictionary *)parameters
{
    ...
    NSString *batchPath = [[TGBRouter sharedInstance] batchPathForPath:path];
...
{
    NSString * myPath = [NSString stringWithFormat:@"/%@/%@", [TGBPaasClient sharedInstance].apiVersion, path];
                              ...
                              parameters:(NSDictionary *)parameters
{
    ...
    if (!URL.scheme.length) {
        NSString *URLString = [[TGBRouter sharedInstance] appURLForPath:path appID:[TGBKKit getApplicationId]];
                              ...
                              block:(AVIdResultBlock)block
{
    ...
    if (parameters && request.URL.absoluteString.length > 4096) {
        NSDictionary *request = [TGBPaasClient batchMethod:@"GET" path:path body:nil parameters:parameters];
        [self postBatchObject:@[request] block:^(NSArray * _Nullable objects, NSError * _Nullable error) {
            if (!error) {
                [TGBUtils callIdResultBlock:block object:objects.firstObject error:nil];
            } else {
...
- (void)getObject:(NSString *)path withParameters:(NSDictionary *)parameters policy:(AVCachePolicy)policy maxCacheAge:(NSTimeInterval)maxCacheAge block:(AVIdResultBlock)block {
    ...
    switch (policy) {
        ...
        {
            [[TGBCacheManager sharedInstance] getWithKey:key maxCacheAge:maxCacheAge block:block];
        }
            break;
        ...
        {
            [[TGBCacheManager sharedInstance] getWithKey:key maxCacheAge:maxCacheAge block:^(id object, NSError *error) {
                if (error) {
                    [self getObjectFromNetworkWithPath:path withParameters:parameters policy:policy block:block];
                } else {
                    block(object, error);
...
-(void)postBatchObject:(NSArray *)requests headerMap:(NSDictionary *)headerMap eventually:(BOOL)isEventually block:(AVArrayResultBlock)block {
    NSString *path = [TGBObjectUtils batchPath];
...
-(void)postBatchSaveObject:(NSArray *)requests headerMap:(NSDictionary *)headerMap eventually:(BOOL)isEventually block:(AVIdResultBlock)block {
    NSString *path = [TGBObjectUtils batchSavePath];
    NSDictionary *parameters = @{@"requests": requests};
    NSMutableURLRequest *request = [self requestWithPath:path method:@"POST" headers:headerMap parameters:parameters];
...
- (void)performRequest:(NSURLRequest *)request saveResult:(BOOL)saveResult block:(AVIdResultBlock)block retryTimes:(NSInteger)retryTimes {
    ...
    if (self.isLastModifyEnabled && [request.HTTPMethod isEqualToString:@"GET"]) {
        ...
        if (modifiedSince && [[TGBCacheManager sharedInstance] hasCacheForKey:URLString]) {
                 ...
                 success:^(NSHTTPURLResponse *response, id responseObject)
    {
        ...
        if (self.isLastModifyEnabled && [request.HTTPMethod isEqualToString:@"GET"]) {
            ...
            if (lastModified && ![self.lastModify[URLMD5] isEqualToString:lastModified]) {
                [[TGBCacheManager sharedInstance] saveJSON:responseObject forKey:URLString];
              ...
              failure:^(NSHTTPURLResponse *response, id responseObject, NSError *error)
    {
        ...
        if (statusCode == 304) {
            // 304 is not error
            [[TGBCacheManager sharedInstance] getWithKey:URLString maxCacheAge:3600 * 24 * 30 block:^(id object, NSError *error) {
                @strongify(self);

                if (error) {
                    if (retryTimes < 3) {
                        [self.lastModify removeObjectForKey:[URLString AVMD5String]];
                        [[TGBCacheManager sharedInstance] clearCacheForKey:URLString];
        ...
        if (error) {
            ...
            if ([NSDictionary lc__checkingType:responseObject]) {
                ...
                if (code) {
                    ...
                    /* for compatibility */
                    id data = error.userInfo[TGBNetworkingOperationFailingURLResponseDataErrorKey];
                    if (data) {
                        userInfo[TGBNetworkingOperationFailingURLResponseDataErrorKey] = data;
                    }
...
//            AVLoggerDebug(AVLoggerDomainNetwork, LC_REST_RESPONSE_LOG_FORMAT, path, costTime, callbackError);
            ...
            if ([self shouldStatisticsForPath:path statusCode:statusCode]) {
                TGBNetworkStatistics *statistician = [TGBNetworkStatistics sharedInstance];
...
- (NSString *)archiveRequest:(NSURLRequest *)request {
    ...
    NSString *fullPath = [[TGBPersistenceUtils eventuallyPath] stringByAppendingPathComponent:fileName];
...
- (void)handleAllArchivedRequests {
    ...
    NSString *documentsDirectory = [TGBPersistenceUtils eventuallyPath];
...
- (TGBACL *)updatedDefaultACL {
    if (self.defaultACL != nil) {
        TGBACL *acl = [self.defaultACL copy];
